...
const validateInvoice = function(invoice: any): boolean {
    if(invoice && invoice.customer_custom_fields && invoice.customer_custom_fields[CUSTOMER_EXTERNAL_FIELD_NAME]){
        if(invoice.type){
            if(INVOICES_TYPES.length > 0){
                if(INVOICES_TYPES.filter( (e: string) => e === invoice.type).length > 0 )
                    return true;
                else {
                    console.log(`Invalid invoice: invoice type ${invoice.type} not created for billing provider (${JSON.stringify(INVOICES_TYPES)}`)
                    return false;
                }
            } 
            else return true;
        ...
        console.log(`Invoice Invalid: missing custom field ${CUSTOMER_EXTERNAL_FIELD_NAME}for billing provider.`)
        return false;
    }
}
